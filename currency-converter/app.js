...
const updateFlag=(element)=>{
    ...
    let Coucode=countrylist[code];
    //here Coudcode is changed
...
const updateExchangeRate=async ()=>{
    ...
    const URL=`${BASE_URL}/${fromcode.value.toLowerCase()}/${tocode.value.toLowerCase()}.json`;
    let response=await fetch(URL);
    //console.log(response);
    let data=await response.json();
    let rate=data[tocode.value.toLowerCase()];
    let finalAmt=amtVal*rate;
    msg.innerText=`${amtVal}${fromcode.value}=${finalAmt}${tocode.value}`;
}